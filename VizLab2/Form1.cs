...
namespace VizLab2
{
    public partial class Form1 : Form
    {
        ...
        // D:\FCIS\4th Year\1st Semester\Scientific & Statistical Visualization\vizlab2\VizLab2\bin\Visualization Data (Meshes)\big.dat
        public Form1()
        {           
            InitializeComponent();
            init();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if(!checkBox1.Checked) //Check if mesh is filled or wired 
            {
                loadMesh_Wired(m);
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            uint varI = Convert.ToUInt32(m.VarToIndex[comboBox2.Text]);
            m.GetMinMaxValues(varI,out minOut,out maxOut);
        ...
        public void mesh_ContourFlooded(Mesh m)
        {
            double Cr, Cg, Cb;
            Gl.glClearColor(1, 1, 1, 1);
            Gl.glClear(Gl.GL_COLOR_BUFFER_BIT | Gl.GL_DEPTH_BUFFER_BIT);
        ...
        private void contoursB_Click(object sender, EventArgs e)
        {
            float contourCount = float.Parse(contourTB.Text);
            contourCount++;
            float s_Val = (float.Parse(maxDataTB.Text) - float.Parse(minDataTB.Text)) / contourCount;
            Gl.glClearColor(1, 1, 1, 1);
            Gl.glClear(Gl.GL_COLOR_BUFFER_BIT | Gl.GL_DEPTH_BUFFER_BIT);
            for (float i = float.Parse(minDataTB.Text); i <= float.Parse(maxDataTB.Text); i += s_Val) //Let i be the value we are searching for
                                                                                                      //i= start range, i<=end range , i+=ratio
                                                                                                      //i+= (max-start/contourcount) garab +1 law feh 7aga 2w -1
                                                                                                      //manfe3sh rbena m3ak yasta :D 
                                                                                                      //SEK 4 LIFE
            {
                foreach (Zone z in m.Zones)
                {
